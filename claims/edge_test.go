...
var _ = Describe("Edge Claims", func() {
	var (
		claims        = newEdgeClaims()
		claimsMap     = newEdgeClaimsMap()
	...
	Context("when creating an EdgeClaimsMap", func() {
		Context("from new", func() {
			It("should contain all edge fields", func() {
				m := NewEdgeClaimsMap()
				_, ok := m["iss"]
				Ω(ok).Should(BeTrue())
				_, ok = m["sub"]
		...
		Context("from a valid EdgeClaims struct", func() {
			It("should be valid", func() {
				err := claims.Validate(validAudience)
				Ω(err).Should(BeNil())
				m := EdgeClaimsFromStruct(claims)
				err = m.Validate(validAudience)
				Ω(err).Should(BeNil())
			})
		})
	})
	Context("when creating a EdgeClaims struct", func() {
		Context("from a valid EdgeClaimsMap", func() {
			It("should be valid", func() {
				err := claimsMap.Validate(validAudience)
				Ω(err).Should(BeNil())
				m := EdgeClaimsFromMap(claimsMap)
				err = m.Validate(validAudience)
				Ω(err).Should(BeNil())
			})
		})
	...
	Context("when validating fields with criteria", func() {
		Context("when using EdgeClaimsMap", func() {
		...
		Context("when using EdgeClaims", func() {
			Context("when the issuer is not valid", func() {
				BeforeEach(func() {
					claims.Iss = "keanu"
				})
